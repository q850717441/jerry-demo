...
public class DateUtil {
     ...
     * 指定日期格式化为字符串
     *
     * @param localDateTime 传入需要格式化的日期 年月日时分秒
     * @param pattern       需要格式化的格式
    ...
    public static String longToString(Long time) {
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern(DEFAULT_PATTERN_DATETIME);
        return dtf.format(LocalDateTime.ofInstant(Instant.ofEpochMilli(time), ZoneId.systemDefault()));
    }
     ...
     * @return 时间戳-Long
     */
    public static Long stringToLong(String str, String pattern) {
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern(pattern);
